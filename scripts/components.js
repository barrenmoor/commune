...
.directive('navbar', function(){
	return {
		...
		template : '<nav class="navbar navbar-default navbar-fixed-top" role="navigation">' +
			...
			'<button type="button" ng-model="selectedTeam" class="btn btn-default navbar-btn">{{selectedTeam}}</button>' +
			'<button type="button" class="btn btn-default navbar-btn dropdown-toggle" data-toggle="dropdown">' +
			'<span class="caret"></span>' +
			'<span class="sr-only">Toggle Dropdown</span>' +
			'</button>' +
			'<ul class="dropdown-menu" role="menu">' +
			'<li ng-repeat="team in teams"><a href="#/selectteam/{{team.id}}" ng-click="changeTeam(team.id)">{{team.name}}</a></li>' +
			'</ul>' +
			'</div>' +
